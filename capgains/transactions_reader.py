        ...
        try:
            with open(csv_file, newline='') as f:
                ...
                for entry_no, entry in enumerate(reader):
                    ...
                    try:
                        entry[Transaction.date_idx] = datetime.strptime(date_str.split(" ")[0],
                                                                        '%Y-%m-%d').date()
                    except ValueError:
                        raise ClickException(
                    "The date ({}) was not entered in the correct format (YYYY-MM-DD)".format(date_str))
                    qty_str = entry[Transaction.qty_idx]
                    try:
                        entry[Transaction.qty_idx] = int(qty_str)
                    except ValueError:
                        raise ClickException(
                    "The quanitity entered {} is not an integer".format(qty_str))
                    price_str = entry[Transaction.price_idx]
                    try:
                        entry[Transaction.price_idx] = float(price_str)
                    except ValueError:
                        raise ClickException(
                    "The price entered {} is not a float value".format(price_str))
                    commission_str = entry[Transaction.commission_idx]
                    try:
                        entry[Transaction.commission_idx] = float(commission_str)
                    except ValueError:
                        raise ClickException(
                    "The commission entered {} is not a float value".format(commission_str))
