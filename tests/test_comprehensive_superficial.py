...
def test_superficial_loss_with_prior_purchase(requests_mock):
    """Test superficial loss when purchase is before the loss (MSFT transactions)"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(requests_mock, date(2022, 1, 1), date(2022, 12, 31))
    
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', os.path.join('tests', 'comprehensive_superficial.csv'), '2022', '-t', 'MSFT', '--format', 'json'])
    ...
    assert len(msft_data['transactions']) == 0  # All transactions should be superficial losses

def test_superficial_loss_with_subsequent_purchase(requests_mock):
    """Test superficial loss when purchase is after the loss (GOOGL transactions)"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(requests_mock, date(2022, 1, 1), date(2022, 12, 31))
    
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', os.path.join('tests', 'comprehensive_superficial.csv'), '2022', '-t', 'GOOGL', '--format', 'json'])
    assert result.exit_code == 0
    ...
    assert len(googl_data['transactions']) == 0  # All transactions should be superficial losses

def test_superficial_loss_edge_case(requests_mock):
    """Test superficial loss with purchase exactly on 30-day boundary (EDGE transactions)"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(requests_mock, date(2024, 1, 1), date(2024, 12, 31))
    
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', os.path.join('tests', 'comprehensive_superficial.csv'), '2024', '-t', 'EDGE', '--format', 'json'])
    assert result.exit_code == 0
...
def test_loss_just_outside_window(requests_mock):
    """Test loss with purchase just outside 30-day window (OUTSIDE transactions)"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(requests_mock, date(2024, 1, 1), date(2024, 12, 31))
    
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', os.path.join('tests', 'comprehensive_superficial.csv'), '2024', '-t', 'OUTSIDE', '--format', 'json'])
    assert result.exit_code == 0
    ...
    assert len(transactions) == 1  # Should have one transaction (not superficial)
...
def test_multiple_purchases_in_window(requests_mock):
    """Test superficial loss with multiple purchases in window (MULTI transactions)"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(requests_mock, date(2024, 1, 1), date(2024, 12, 31))
    
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', os.path.join('tests', 'comprehensive_superficial.csv'), '2024', '-t', 'MULTI', '--format', 'json'])
    assert result.exit_code == 0
...
def test_zero_balance_at_window_end(requests_mock):
    """Test that a loss is not superficial if share balance is 0 at end of window (ZERO transactions)"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(requests_mock, date(2024, 1, 1), date(2024, 12, 31))
    
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', os.path.join('tests', 'comprehensive_superficial.csv'), '2024', '-t', 'ZERO', '--format', 'json'])
    assert result.exit_code == 0
    ...
    # First transaction should be a regular loss (not superficial) since share balance is 0 at end of window
...
def test_different_ticker_in_window(requests_mock):
    """Test that purchases of different tickers don't affect superficial loss (DIFF transactions)"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(requests_mock, date(2024, 1, 1), date(2024, 12, 31))
    
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', os.path.join('tests', 'comprehensive_superficial.csv'), '2024', '-t', 'DIFF', '--format', 'json'])
    assert result.exit_code == 0
    ...
    assert len(diff_data['transactions']) == 0  # Should be a superficial loss since there's a buy of DIFF within the window 