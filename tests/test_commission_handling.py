...
from tests.helpers import create_csv_file, create_json_file
...
def test_zero_commission_transactions(requests_mock, tmpdir):
    """Test transactions with zero commissions to ensure they're handled correctly"""
    ...
    transactions = [
        ...
        ["2023-01-15", "Zero Commission Buy", "ZERO", "BUY", "100", "50.00", "0.00", "USD"],
        # Sell 50 shares with no commission
        ["2023-02-15", "Zero Commission Sell", "ZERO", "SELL", "50", "55.00", "0.00", "USD"]
    ]
    
    csv_path = create_csv_file(tmpdir, "test_zero_commission.csv", transactions)
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', csv_path, '2023', '-t', 'ZERO', '--format', 'json'])
...
def test_small_commission_rounding(requests_mock, tmpdir):
    """Test transactions with very small commissions to verify rounding behavior"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(requests_mock, date(2023, 1, 1), date(2023, 12, 31))
    
    transactions = [
        # Buy with small commission
        ["2023-01-15", "Small Commission Buy", "SMALL", "BUY", "100", "50.00", "0.01", "USD"],
        # Sell with small commission
        ["2023-02-15", "Small Commission Sell", "SMALL", "SELL", "50", "55.00", "0.005", "USD"]
    ]
    
    csv_path = create_csv_file(tmpdir, "test_small_commission.csv", transactions)
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', csv_path, '2023', '-t', 'SMALL', '--format', 'json'])
    assert result.exit_code == 0
...
def test_same_day_mixed_currency_commission(requests_mock, tmpdir):
    """Test commission handling for multiple currency transactions on the same day"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(requests_mock, date(2023, 1, 1), date(2023, 12, 31))
    
    transactions = [
        # Buy USD stock with USD commission
        ["2023-01-15", "USD Buy", "MIXED", "BUY", "100", "50.00", "9.99", "USD"],
        # Buy CAD stock with CAD commission
        ["2023-01-15", "CAD Buy", "XIU.TO", "BUY", "200", "25.00", "9.99", "CAD"],
        # Sell USD stock with USD commission
        ["2023-02-15", "USD Sell", "MIXED", "SELL", "50", "55.00", "9.99", "USD"],
        # Sell CAD stock with CAD commission
        ["2023-02-15", "CAD Sell", "XIU.TO", "SELL", "100", "27.00", "9.99", "CAD"]
    ]
    
    csv_path = create_csv_file(tmpdir, "test_mixed_currency.csv", transactions)
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', csv_path, '2023', '--format', 'json'])
    assert result.exit_code == 0
    ...
    mixed_tx = next(tx for tx in data['MIXED']['transactions'] if tx['description'] == 'USD Sell')
    ...
    xiu_tx = next(tx for tx in data['XIU.TO']['transactions'] if 'CAD Sell' in tx['description'])
...
def test_commission_in_superficial_loss(requests_mock, tmpdir):
    ...
    transactions = [
        # Initial buy
        ["2023-01-15", "Initial Buy", "SUPER", "BUY", "100", "50.00", "9.99", "USD"],
        # Sell at a loss
        ["2023-02-15", "Loss Sale", "SUPER", "SELL", "50", "40.00", "9.99", "USD"],
        # Buy back within 30 days (triggering superficial loss)
        ["2023-03-01", "Buy Back", "SUPER", "BUY", "50", "45.00", "9.99", "USD"]
    ]
    
    csv_path = create_csv_file(tmpdir, "test_superficial_loss.csv", transactions)
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', csv_path, '2023', '-t', 'SUPER', '--format', 'json'])
    assert result.exit_code == 0
    ...
    # The transaction should not appear in the output since it's a superficial loss
