...
def test_regular_gains_and_losses(requests_mock):
    ...
    assert abs(aapl_data['total_gains'] - 9320.055) < 0.01
    ...
    gain_tx = next(
        tx for tx in transactions if 'Regular Gain AAPL' in tx['description']
    )
    loss_tx = next(
        tx for tx in transactions if 'Regular Loss AAPL' in tx['description']
    )
    ...
    # 23000.00 - 14259.99 - 19.98
    assert abs(gain_tx['capital_gain'] - 8720.03) < 0.01
    ...
    # 7750.00 - 7129.995 - 19.98
    assert abs(loss_tx['capital_gain'] - 600.025) < 0.01
...
def test_partial_share_transactions(requests_mock):
    ...
    # Same as transaction capital gain
    assert abs(vfv_data['total_gains'] + 9.735) < 0.01


def test_complete_sale_loss(requests_mock):
    """Test loss that's not superficial due to complete sale (TD.TO
    transactions)"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(
        requests_mock, date(2022, 1, 1), date(2022, 12, 31)
    ...
    assert abs(tx['outlays'] - 9.99) < 0.01  # Commission in CAD
    # 19750.00 - 21259.99 - 9.99
    assert abs(tx['capital_gain'] + 1519.98) < 0.01

    # Same as transaction capital loss
    assert abs(td_data['total_gains'] + 1519.98) < 0.01


def test_mixed_currency_transactions(requests_mock):
    """Test gains/losses with mixed currency transactions (META
    transactions)"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(
    ...
    # Find gain and loss transactions
    gain_tx = next(
        tx for tx in transactions if 'Quick Gain Meta' in tx['description']
    )
    loss_tx = next(
        tx for tx in transactions
        if 'Loss with Mixed Currency' in tx['description']
    )
    ...
    assert abs(gain_tx['outlays'] - 19.98) < 0.01  # 9.99 × 2.0
    # 24200.00 - 20009.99 - 19.98
    assert abs(gain_tx['capital_gain'] - 4170.03) < 0.01
    ...
    # 4750.00 - 10004.995 - 9.99
    assert abs(loss_tx['capital_gain'] + 5264.985) < 0.01
...
def test_regular_cad_transactions(requests_mock):
    ...
    assert abs(tx['outlays'] - 9.99) < 0.01  # Commission in CAD
    # 5500.00 - 5103.996 - 9.99
    assert abs(tx['capital_gain'] - 386.014) < 0.01

    # Same as transaction capital gain
    assert abs(xiu_data['total_gains'] - 386.014) < 0.01
