...
def test_can_read_sample_files():
    ...
    # Test CSV file
    csv_transactions = TransactionsReader.get_transactions(os.path.join('tests', 'sample.csv'))
    assert len(csv_transactions) == 36
    
    # Test JSON file
    json_transactions = TransactionsReader.get_transactions(os.path.join('tests', 'sample.json'))
...
def test_maxcost_sample_2022():
    ...
    # Test CSV input
    csv_result = runner.invoke(capgains, ['maxcost', os.path.join('tests', 'sample.csv'), '2022'])
    assert csv_result.exit_code == 0
    
    # Test JSON input
    json_result = runner.invoke(capgains, ['maxcost', os.path.join('tests', 'sample.json'), '2022'])
...
def test_maxcost_sample_2023():
    ...
    # Test CSV input
    csv_result = runner.invoke(capgains, ['maxcost', os.path.join('tests', 'sample.csv'), '2023'])
    assert csv_result.exit_code == 0
    
    # Test JSON input
    json_result = runner.invoke(capgains, ['maxcost', os.path.join('tests', 'sample.json'), '2023'])
    assert json_result.exit_code == 0
...
def test_maxcost_sample_2024():
    ...
    # Test CSV input
    csv_result = runner.invoke(capgains, ['maxcost', os.path.join('tests', 'sample.csv'), '2024'])
    assert csv_result.exit_code == 0
    
    # Test JSON input
    json_result = runner.invoke(capgains, ['maxcost', os.path.join('tests', 'sample.json'), '2024'])
    assert json_result.exit_code == 0
...
def test_calc_sample_2022():
    ...
    # Test CSV input
    csv_result = runner.invoke(capgains, ['calc', os.path.join('tests', 'sample.csv'), '2022'])
    assert csv_result.exit_code == 0
    
    # Test JSON input
    json_result = runner.invoke(capgains, ['calc', os.path.join('tests', 'sample.json'), '2022'])
    assert json_result.exit_code == 0
...
def test_calc_sample_2023():
    ...
    # Test CSV input
    csv_result = runner.invoke(capgains, ['calc', os.path.join('tests', 'sample.csv'), '2023'])
    assert csv_result.exit_code == 0
    
    # Test JSON input
    json_result = runner.invoke(capgains, ['calc', os.path.join('tests', 'sample.json'), '2023'])
    assert json_result.exit_code == 0
...
def test_calc_sample_2024():
    ...
    # Test CSV input
    csv_result = runner.invoke(capgains, ['calc', os.path.join('tests', 'sample.csv'), '2024'])
    assert csv_result.exit_code == 0
    
    # Test JSON input
    json_result = runner.invoke(capgains, ['calc', os.path.join('tests', 'sample.json'), '2024'])
    assert json_result.exit_code == 0
...
def test_show_sample():
    ...
    csv_result = runner.invoke(capgains, ['show', os.path.join('tests', 'sample.csv')])
    assert csv_result.exit_code == 0
    
    # Test showing all transactions from JSON
    json_result = runner.invoke(capgains, ['show', os.path.join('tests', 'sample.json')])
    assert json_result.exit_code == 0
    ...
    csv_filtered = runner.invoke(capgains, ['show', os.path.join('tests', 'sample.csv'), '-t', 'AAPL'])
    json_filtered = runner.invoke(capgains, ['show', os.path.join('tests', 'sample.json'), '-t', 'AAPL'])
    ...
    csv_rates = runner.invoke(capgains, ['show', os.path.join('tests', 'sample.csv'), '-e'])
    json_rates = runner.invoke(capgains, ['show', os.path.join('tests', 'sample.json'), '-e'])
