...
def test_csv_header_detection(tmpdir):
    ...
    with pytest.raises(ClickException,
                       match="First row appears to be a header row"):
...
def test_invalid_numeric_values(tmpdir):
    ...
    with pytest.raises(ClickException,
                       match="quantity entered .* is not a valid number"):
        TransactionsReader.get_transactions(str(csv_file))
...
def test_csv_header_detection_with_date_field(tmp_path):
    ...
    with open(csv_path, "w") as f:
        header = (
            "date,description,ticker,action,quantity,price,"
            "commission,currency\n"
        )
    ...
    error_msg = (
        "First row appears to be a header row. The CSV file should "
        "contain only transaction data without headers.\nEach row "
        "should be in the format: YYYY-MM-DD,description,ticker,"
        "action,quantity,price,commission,currency"
    )
