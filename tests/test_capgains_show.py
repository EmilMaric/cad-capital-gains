...
def test_ticker(transactions, capfd):
    ...
    CapGainsShow.capgains_show(transactions, show_exchange_rate=False, tickers=['ANET'])
    out, _ = capfd.readouterr()
    assert out == """\
+------------+---------------+----------+----------+-------+---------+--------------+------------+
| date       | description   | ticker   | action   |   qty |   price |   commission |   currency |
...
def test_unknown_ticker(transactions, capfd):
    ...
    transactions
    """
    CapGainsShow.capgains_show(transactions, show_exchange_rate=False, tickers=['FB'])
    out, _ = capfd.readouterr()
    assert out == """\
No results found
"""
...
def test_known_ticker_and_unknown_ticker(transactions, capfd):
    ...
    """
    CapGainsShow.capgains_show(transactions, show_exchange_rate=False, tickers=['GOOGL', 'FB'])
    out, _ = capfd.readouterr()
    assert out == """\
+------------+---------------+----------+----------+-------+---------+--------------+------------+
| date       | description   | ticker   | action   |   qty |   price |   commission |   currency |
...
def test_multiple_tickers(transactions, capfd):
    ...
    transactions
    """
    CapGainsShow.capgains_show(transactions, show_exchange_rate=False, tickers=['ANET', 'GOOGL'])
    out, _ = capfd.readouterr()
    assert out == """\
+------------+---------------+----------+----------+-------+---------+--------------+------------+
| date       | description   | ticker   | action   |   qty |   price |   commission |   currency |
