...
def test_overlapping_loss_windows(requests_mock):
    """Test realistic overlapping superficial loss windows where trades happen naturally close together"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(requests_mock, date(2024, 1, 1), date(2024, 12, 31))
    
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', os.path.join('tests', 'comprehensive_edge_cases.csv'), '2024', '-t', 'OVERLAP', '--format', 'json'])
    ...
    # Capital loss = $5,400.00 - $19.98 - $6,006.00 = -$625.98 CAD (superficial)
    ...
    # Buy on Feb 10 falls within both windows and we still own shares after both windows

def test_invalid_transaction_protection(requests_mock):
    """Test that the code protects against invalid transactions that would create negative balances"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(requests_mock, date(2024, 1, 1), date(2024, 12, 31))
    
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', os.path.join('tests', 'comprehensive_edge_cases.csv'), '2024', '-t', 'PROTECT', '--format', 'json'])
...
def test_natural_zero_balance(requests_mock):
    """Test a realistic sequence of trades that happens to end at zero balance"""
    # Set up exchange rate mock for the date range
    setup_exchange_rates_mock(requests_mock, date(2024, 1, 1), date(2024, 12, 31))
    
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', os.path.join('tests', 'comprehensive_edge_cases.csv'), '2024', '-t', 'EXACT', '--format', 'json'])
    assert result.exit_code == 0
    ...
    first_sell = next(tx for tx in transactions if tx['description'] == 'Small Sell 1')
    ...
    assert abs(first_sell['capital_gain'] + 325.98) < 0.01  # $4,200.00 - $19.98 - $4,506.00
    
    # All losses should be regular since we end with zero balance
    sell_transactions = [tx for tx in transactions if tx['description'].startswith('Small Sell') or tx['description'] == 'Final Sell']
    assert len(sell_transactions) == 3
    for tx in sell_transactions:
        assert tx['capital_gain'] < 0  # Should be regular losses, not superficial
...
def test_window_boundary_trades(requests_mock):
    ...
    runner = CliRunner()
    result = runner.invoke(capgains, ['calc', os.path.join('tests', 'comprehensive_edge_cases.csv'), '2024', '-t', 'WINDOW', '--format', 'json'])
    assert result.exit_code == 0
    ...
    # Capital loss = $14,400.00 - $19.98 - $16,008.00 = -$1,627.98 CAD (superficial)
